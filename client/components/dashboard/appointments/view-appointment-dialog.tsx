      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle className="flex items-center justify-between pr-6">
            Appointment Details
            <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase border ${statusVariants[appointment.status]}`}>
              {appointment.status}
            </span>
          </DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-6 pt-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm font-medium" suppressHydrationWarning>{format(new Date(appointment.appointment_date), "PPP")}</p>
          ...
          <div className="pt-4 border-t flex justify-between items-center text-[10px] text-muted-foreground">
            ...
            <span suppressHydrationWarning>Created on {format(new Date(appointment.created_at), "MMM d, yyyy")}</span>
          </div>
        </div>
      </DialogContent>
    </Dialog>
